...
function ToDoFormAndList() {
  ...
  // or Javascript - lamda or arrow function
  const removeItem = (id) => {
    // filtter / remove item with id
    ...
    <div>
      <form onSubmit={handleSubmit}>
        <input type='text' 
          ...
          placeholder='Write a new todo here'></input>
        <input type='submit' value='Add'></input>
      </form>
      <ul>
        {items.map(item => (<li key={item.id}>{item.text+" "} <span onClick={() => removeItem(item.id)}>x</span></li>))}
      </ul>
    </div>
  )
}
...
function App() {
  return (
    <div>
      <Banner></Banner>
      <ToDoFormAndList></ToDoFormAndList>
    </div>
  );
}
